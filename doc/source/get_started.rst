...
- the CoSMoMVPA Matlab source code:
  ...
  In Matlab_, run ``cosmo_set_path`` (in the ``mvpa/`` directory) followed by ``savepath``. Alternatively one can manually add the ``mvpa`` and ``external`` directories (and their subdirectories) to the Matlab path.
