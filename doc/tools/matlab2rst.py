...
def matlab2rst(data, output=None):
    ...
    for i, line in enumerate(lines):
        ...
        add_line = (
            (output is None)
            or (output == "skl" and not in_skeleton)
            or (output in ("hdr", "sgn") and not after_header)
        )
...
class CoSMoModules(object):
    _name2funcs = dict(
        ...
        visualization=["disp", "plot_slices"],
        datasets=[
            "fmri_dataset",
            "map2fmri",
            "meeg_dataset",
        ...
        stats=["statcode", "stat", "anova_feature_selector"],
        correlations=["correlation_measure", "corr"],
        rsa=[
            "target_dsm_corr_measure",
...
for output in ("hdr", "skl", None):
    ...
    for rst_type in rst_types:
        ...
        if rebuild_toc:
            ...
            toctree_body = "\n".join(
                "    %s/%s" % (output_mat_rel, b) for b, _ in base_names
            )
            header = "\n".join(
                [ref_header, title, "", toctree_header, toctree_body, "", ""]
            )
