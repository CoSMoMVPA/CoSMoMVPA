...
class Image(object):
    ...
    def get_image_ref(self):
        ...
        root_dir = os.path.join(os.path.dirname(__file__), relative_to_image_dir)
...
class Cell(object):
    ...
    def tabelize(cls, cells, n_columns, hor_sep, ver_sep, edge_sep):
        ...
        filled_cells = [
            cell.fill_horizontally(max_width).fill_vertically(max_height)
...
class ImageCollection(object):
    ...
    def get_rst_table(self, n_columns=4):
        ...
        return str(
            Cell.tabelize(
                cells, n_columns=n_columns, hor_sep="-", ver_sep="|", edge_sep="+"
            )
        )
    ...
    def write(self, fn=None):
        if fn is None:
            ...
            fn = os.path.join(os.path.dirname(__file__), relative_fn)
...
if __name__ == "__main__":
    demos = {
        "fMRI ROI classification analysis": ("demo_fmri_rois", 8),
        "fMRI ROI split-half correlations": ("run_splithalf_correlations", 5),
        "fMRI split-half correlations searchlight": (
            "demo_fmri_correlation_searchlight",
            1,
        ),
        "Surface-based classification searchlight": ("demo_surface_searchlight_lda", 1),
        "Representational similarity analysis searchlight": (
            "demo_fmri_searchlight_rsm",
            2,
        ),
        "Fast Naive Bayes searchlight": ("demo_fmri_searchlight_naive_bayes", 1),
        "DISTATIS": ("demo_fmri_distatis", 1),
        "Threshold-Free Cluster Enhancement": ("demo_surface_tfce", 2),
        "MEEG timeseries classification": ("demo_meeg_timeseries_classification", 1),
        "MEEG time-locked searchlight": ("demo_meeg_timelock_searchlight", 1),
        "MEEG time-frequency searchlight": ("demo_meeg_timefreq_searchlight", 1),
        "MEEG time generalization": ("demo_meeg_timeseries_generalization", 1),
    ...
    if n_images == 0:
        msg = (
            "No elements found for gallery. To build the gallery, "
