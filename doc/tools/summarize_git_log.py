...
def build_git_log(log_fn=log_fn, since=git_since):
    ...
    if git_log_out_of_date(log_fn=log_fn):
        ...
        print(
            ("rebuilding git log file: commits were " "made since last update . . ."),
            end=" ",
        )
...
def line_has_tag(line, tag):
    ...
    return line.startswith("   ") and tag in line.split(":")[0].strip().split("+")
...
def element(header, body_parts, sep="\n"):
    ...
    is_sequence = isinstance(body_parts, (set, list))
    is_empty = (not body_parts) if is_sequence else (not len(body_parts.strip()))
    if is_empty:
        # Nothing to show
        return ""
    ...
    body = sep + (sep.join(body_parts) if is_sequence else body_parts)
...
class CommitFileChanged(object):
    ...
    def is_linkable(self):
        subdirs2prefix = dict(mvpa=["cosmo"], tests=["test_"], examples=None)
        fn = self.filename.strip()
        path_elements = fn.split(os.path.sep)

        in_linkable_dir = path_elements[0] in subdirs2prefix and len(path_elements) > 1
        ...
        has_prefix = allowed_prefixes is None or any(
            name.startswith(ap) for ap in allowed_prefixes
        )
...
class CommitLogEntry(object):
    ...
    def from_lines(lines, skip=0):
        ...
        files_changed = [CommitFileChanged.from_line(line) for line in files]
    ...
    def rst_message(self):
        ...
        width = 70
        return textwrap.fill(m, width=width, subsequent_indent=" " * (indent_count + 4))

    def rst_str(self):
        if self.has_stats():
            files_lines = [f.rst_str() for f in self.files_changed] + self.stats + [""]
        else:
            files_lines = []

        lines = [self.rst_preamble(), "", self.rst_message(), ""] + files_lines + [""]
