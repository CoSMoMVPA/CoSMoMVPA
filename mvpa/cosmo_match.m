...
%
% msk=cosmo_match(needle1, haystack1, [needle2, haystack2, ...)
%
% Inputs:
...
%                     
% Examples (in all cases, ds is assumed to be a dataset struct):
...
%
% See also: cosmo_slice, cosmo_stack
%
% NNO Sep 2013
...
if isnumeric(haystack) && isnumeric(needle) && numel(needle)==1
    ...
    switch tp
        ...
        case 'cell_with_strings'
            ...
            for k=1:nrows
                match_indices=strmatch(needle{k}, haystack, 'exact');
...
for k=1:numel(fns)
    ...
    if check_func(needle) 
        if check_func(haystack)
            ...
            error('first argument type %s but second is not',fn);
        end
    end
end

error('Unsupported type; type not one of %s', sprintf('%s ', fns{:}));